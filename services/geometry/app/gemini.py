...
SYSTEM = (
    ...
    "realistic-metre ESTIMATES, not hard limits: the solver rescales the targets "
    "to sum to footprint_target_m2 (holding the garage) and treats any min above "
    "the Neufert floor as a soft preference, so approximate values are fine — but "
    "keep composites (master_suite, children, kitchen_laundry, entry) roomy "
    "enough to subdivide (children min_h_m>=6, kitchen_laundry and entry "
    "min>=3-4 m). (4) categories: living=living/dining, private=bedrooms, "
